...
export class MatchService {
  ...
  async createMatch(payload, userId) {
    ...
    const matchData = {
      ...payload,
      oversPerInnings:
        payload.oversPerInnings ?? MATCH_OVERS[payload.matchType],
      createdBy: userId,
    };
  ...
  async updateMatch(matchId, payload, userId) {
    ...
    let updatedMatch = await this.matchRepository.updateById(
      matchId,
      updateData,
    );
  ...
  async publishMatch(matchId, userId) {
    ...
    if (match.status !== MATCH_STATUS.DRAFT) {
      throw new ConflictError("Only draft matches can be published");
    }
  ...
  async updateToss(matchId, payload, userId) {
    ...
    if (match.status !== MATCH_STATUS.UPCOMING) {
      throw new ConflictError("Toss can only be updated for upcoming matches");
    }
  ...
  async startMatch(matchId, userId) {
    ...
    const canUseSquadXI =
      team1SquadXI.length === 11 && team2SquadXI.length === 11;

    if (
      match.status !== MATCH_STATUS.PLAYING_XI_SELECTED &&
      !(
        match.status === MATCH_STATUS.TOSS_COMPLETED &&
        (hasPlayingXI || canUseSquadXI)
      )
    ) {
      throw new ConflictError(
  ...
  async pauseForInningsBreak(matchId, userId) {
    ...
    if (match.status !== MATCH_STATUS.LIVE) {
      throw new ConflictError("Only live matches can enter innings break");
    }

    const firstInnings = await this.inningsRepository.findByMatchAndNumber(
      matchId,
      1,
    );
    if (!firstInnings || firstInnings.status !== INNINGS_STATUS.COMPLETED) {
      throw new ConflictError(
        "First innings must be completed before innings break",
      );
    }
  ...
  async startSecondInnings(matchId, userId) {
    ...
    if (
      ...
      ![1, 2].includes(match.currentInnings)
    ) {
      throw new ConflictError("Match is not currently in innings break");
    }
  ...
  async validatePlayingXI(match, payload) {
    ...
    const team1Players = new Set(
      payload.team1.map(({ player }) => String(player)),
    );
  ...
  validateTeamPlayingXI(playingXI, team, label) {
    ...
    if (uniquePlayerIds.size !== 11) {
      throw new BadRequest(
        `${label} Playing XI must contain 11 unique players`,
      );
    }
    ...
    if (playerIds.some((playerId) => !squadPlayerIds.has(playerId))) {
      throw new BadRequest(
        `${label} Playing XI contains a player outside the team squad`,
      );
    }
    ...
    if (captainCount !== 1) {
      throw new BadRequest(
        `${label} Playing XI must contain exactly one captain`,
      );
    }

    if (wicketKeeperCount !== 1) {
      throw new BadRequest(
        `${label} Playing XI must contain exactly one wicketkeeper`,
      );
    }
  }
  ...
  async abandonMatch(matchId, userId) {
    ...
    if (
      ...
      throw new ConflictError("Match cannot be abandoned");
    }
  ...
  async markNoResult(matchId, userId) {
    ...
    if (
      ...
      throw new ConflictError("Cannot mark completed match as no result");
    }
  ...
  async completeMatch(matchId, payload, userId) {
    ...
    if (
      ...
      throw new ConflictError("Only live matches can be completed");
    }

    const secondInnings = await this.inningsRepository.findByMatchAndNumber(
      matchId,
      2,
    );
    if (!secondInnings || secondInnings.status !== INNINGS_STATUS.COMPLETED) {
      throw new ConflictError(
        "Second innings must be completed before completing the match",
      );
    }
