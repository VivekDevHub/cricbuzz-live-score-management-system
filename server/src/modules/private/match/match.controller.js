...
export default class MatchController {
  ...
  createMatch = async (req, res) => {
    const match = await this.matchService.createMatch(req.body, req.user.id);
  ...
  deleteMatch = async (req, res) => {
    const match = await this.matchService.deleteMatch(
      req.params.id,
      req.user.id,
    );
  ...
  publishMatch = async (req, res) => {
    const match = await this.matchService.publishMatch(
      req.params.id,
      req.user.id,
    );
  ...
  updateToss = async (req, res) => {
    const match = await this.matchService.updateToss(
      req.params.id,
      req.body,
      req.user.id,
    );
  ...
  selectPlayingXI = async (req, res) => {
    const match = await this.matchService.selectPlayingXI(
      req.params.id,
      req.body,
      req.user.id,
    );
    ...
    return ApiResponse(
      res,
      200,
      "Match status and playing-XI updated successfully",
      match,
    );
  };
  ...
  pauseForInningsBreak = async (req, res) => {
    const match = await this.matchService.pauseForInningsBreak(
      req.params.id,
      req.user.id,
    );
  ...
  startSecondInnings = async (req, res) => {
    const match = await this.matchService.startSecondInnings(
      req.params.id,
      req.user.id,
    );
  ...
  abandonMatch = async (req, res) => {
    const match = await this.matchService.abandonMatch(
      req.params.id,
      req.user.id,
    );
  ...
  markNoResult = async (req, res) => {
    const match = await this.matchService.markNoResult(
      req.params.id,
      req.user.id,
    );
